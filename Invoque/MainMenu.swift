...
/// Invoque's main menu, which exists almost entirely so text fields work.
///
...
/// them — and the launcher panel's search field is exactly that kind of field.
///
...
/// an Invoque text field is focused.
enum MainMenu {
    ...
    static func install(into application: NSApplication) {
        ...
        // `windowsMenu` is deliberately left unset. Handing AppKit the Window
        // menu makes it list every window Invoque owns, and the launcher panel
        // is not the kind anyone should be offered. The three items below reach
        // the focused window through the responder chain without any of that.
    }
...
private extension NSApplication {
    ...
    var appName: String {
        Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "Invoque"
    }
}
